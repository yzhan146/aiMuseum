...
- 对话改为 LLM-first；无 Claim 时允许人物边界内的模型角色演绎，有 Claim 时附加馆藏引用；
- 为36位人物增加身份、知识领域、时代、语气、情绪范围、表达习惯与拒答风格；
- 将 Catalog 关系写入人物包一跳关系图，并区分公开来源确认与模型建议关系；
- 增加时代错位概念检查，例如李白不会以现代知识解释微积分；
...
- 35个新增人物包仍只含最小馆藏 Claim；人物可以进行模型角色演绎，但这些回答不等于史料核验，也不满足公开发布所需的20条 Claim、3个来源和完整评测门槛；
