...
export function classifyQuestion(pack: CharacterPack, message: string): Omit<KnowledgeSelection, "entities" | "claims"> {
  ...
  if (mentionsAfterCutoff(message, pack.boundaries.knowledgeCutoff) || pack.boundaries.unknownPatterns.some(term => text.includes(normalize(term)))) return { classification: "after-lifetime", boundary: true, reason: `人物认知截止于 ${pack.boundaries.knowledgeCutoff}` };
