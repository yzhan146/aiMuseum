...
### PK-UX-01：统一公开知识站与登录产品的视觉语言

- 状态：待规划；不阻塞当前公开知识与指标 MVP。
- 问题：公开知识站采用米白、墨绿和编辑型版式，登录后的博物馆采用蓝色、卡片化和沉浸式界面；能力边界合理，但视觉跳变可能造成品牌割裂与登录后的心理落差。
- 方向：保留公开只读与登录互动的能力差异，统一品牌色、字体、导航、按钮、人物卡、时期主题和空间隐喻；公开入口应像同一座博物馆的大厅，而不是另一个产品。
- 边界：不重新引入完整游客模式；儿童/成人视觉模式继续属于登录后的沉浸体验。
- 验收：用户能够从公开首页进入人物、时期、登录和博物馆页面，并始终明确自己仍处于同一产品；桌面与移动端均通过视觉连续性评审。
