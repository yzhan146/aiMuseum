...
export async function sendThreadMessage(userId: string, thread: ConversationThread, content: string, ageBand = "9-12", locale = "zh-CN"): Promise<ChatResult> {
  ...
  const userMessage = await addMessage(thread, { role: "user", content, claimIds: [], citations: [] });
  const memories = await activeCharacterMemories(userId, thread.characterId); const mastery = await listMastery(userId);
  ...
  for (const callback of result.memoryCallbacks ?? []) { const memory = memories.find(item => item.id === callback.memoryId); if (memory) await recallMemory(memory); }
  const characterMessage = await addMessage(thread, { role: "character", content: result.answer, claimIds: result.claimIds, citations: result.citations });
  ...
  for (const topic of topics) await exposeTopic(userId, topic, [userMessage.id, characterMessage.id]);
  if (!result.boundary && result.claimIds.length) {
    const entities = claims.flatMap(claim => [claim.subjectId, claim.objectId].filter(Boolean)).map(id => pack.entities.find(entity => entity.id === id)?.names[pack.manifest.defaultLocale]).filter(Boolean);
    const labels = [...new Set([...entities, ...topics])].slice(0, 6);
    await createMemory({ userId, characterId: thread.characterId, type: "character_relationship", content: `我们曾聊过：${labels.join("、")}`, sourceMessageIds: [userMessage.id, characterMessage.id], confidence: .8, importance: .6, sensitivity: "low", status: "active" });
  }
  const freshMastery = await listMastery(userId); const exposed = topics.map(topic => freshMastery.find(item => item.topicId === topic)).find(Boolean);
