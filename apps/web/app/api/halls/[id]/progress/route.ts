...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    const completedAllStations = pack.hall.stations.every((station) => completedStationIds.includes(station.id));
    const update = { userId: identity.userId, hallId: id, sceneVersion: pack.hall.sceneVersion, lastStationId: input.lastStationId, viewedObjectIds, visitedCharacterIds, completedStationIds, completedAt: completedAllStations ? new Date().toISOString() : undefined };
